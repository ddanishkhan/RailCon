<div class="card-body">
  ...
  <form action="bulkUpdate.php" method="POST" id="bulkIssueForm">
    ...
    <div id="bulkActionBar" class="mb-3 px-3 py-2 bg-light border rounded align-items-center" style="display:none">
      ...
      <button type="submit" name="bulkIssueSubmit" class="btn btn-success btn-sm mr-2"
        ...
        if ($result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
            ...
            <tr id="row_<?= $idd ?>">
              ...
              <td>
                <img id="img<?= $idd ?>" src="MyUploadImages/<?= htmlspecialchars($row['img_loc']) ?>" data-toggle="modal"
                  data-target="#imgModal" data-student="<?= htmlspecialchars($row['fullname']) ?>"
              ...
              <td>
                <div class="custom-control custom-checkbox">
                  <input type="checkbox" class="custom-control-input bulk-checkbox" id="bulk_<?= $idd ?>"
                    value="<?= $idd ?>">
