  ...
  if($filter == "Males")
	{
		...
		$sql_display = "SELECT id, fullname, source, destination, passno, classof, duration, img_loc FROM student WHERE gender=0 AND verified=0";
        $result = $db->query($sql_display);
	if ($result->num_rows > 0) {
  ...
  if($filter == "Females")
	{
		...
		$sql_display = "SELECT id, fullname, source, destination, passno, classof, duration, img_loc FROM student WHERE gender=1 AND verified=0";
        $result = $db->query($sql_display);
	if ($result->num_rows > 0) {
